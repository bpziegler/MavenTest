	...
	private void processFile(File oneFile, int fileNum) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (now - lastLog >= 250) {
				...
				double linesPerSec = (0.0 + curLine) / elapSec;
				String durStr = DurationFormatUtils.formatDuration((long) remainTime * 1000, "H:mm:ss");
				String extraStatus = lineProcessor.getStatus();
				
				String status = String.format("File %2d   Line %,12d   Elap %,8.1f   Remain %8s   %7.3f %%   MB %,8d   Line/Sec %,8.0f   %s", 
						fileNum, curLine, elapSec, durStr, 100.0 * curBytes / totBytes, usedMB, linesPerSec, extraStatus);
